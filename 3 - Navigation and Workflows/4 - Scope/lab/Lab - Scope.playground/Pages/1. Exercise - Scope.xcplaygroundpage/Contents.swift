...
//print("The value of foo is \(foo)")
// foo is inside the loop's scope and does not exist once the loop is complete (i.e. after the trailing '}' that defines the loop)
...
// x is defined outside of the loop's scope and thus will exist afterward, additionaly, changes inside the loop will be retained by 'x'.
// foo was defined in the local scope of the loop above.
...
greeting(greeting: nil, name: "Joe")
...
let ferrari = Car(make: "Ferrari", model: "La Ferrari", year: 2018)
let carolla = Car(make: "Toyta", model: "Carolla", year: 2019)
